 ...
 * The filename may contain 'a-zA-Z_.' but no '/'. If path is omitted it will be assumed to be "include".
 * The preprocessor will then attempt to load the file "[resourcePath]{path}/{filename}".
 ...
 * For this pass the corresponding [Redefine] to the constructor.
 *
 * @author Aton
 */
class ShaderPreprocessor(private val resourcePath: String) {

    constructor(resourcePath: String, args: Collection<PreprocessorArgument>) : this(resourcePath) {
        args.forEach {
            when(it) {
    ...
    override fun equals(other: Any?): Boolean {
        return other is ShaderPreprocessor && resourcePath == other.resourcePath
