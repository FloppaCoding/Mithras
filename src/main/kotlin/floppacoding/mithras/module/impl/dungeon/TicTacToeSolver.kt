...
object TicTacToeSolver : Module(
    ...
    description = "Solves the tic tac toe puzzle."
) {
    private val outlineColor by ColorSetting("Outline Color", Color(20, 200, 70), description = "Color of the outline for the correct button.")
    private val fillColor by ColorSetting("Fill Color", Color(20, 200, 70,100), description = "Fill Color for the button sides.")
    ...
    fun onTick(event: ClientTickEvent) {
        if (!inDungeons || event.phase != ClientTickEvent.Phase.START || mc.player == null || mc.world == null) return
        ...
        val frames = mc.world!!.getEntitiesByClass(ItemFrameEntity::class.java, tttSerachBox) filter@{
            ...
            if (itemFrameHeldStack == null || itemFrameHeldStack.item != Items.FILLED_MAP) return@filter false
            val mapData = FilledMapItem.getMapState(itemFrameHeldStack, mc.world) ?: return@filter false
            ...
            if (mc.world!!.getBlockState(blockBehind).block != Blocks.IRON_BLOCK) return@filter false
            return@filter true
        }
        try {
            if (topLeft == null || roomFacing == null || board == null) {
                for (frame in frames) {
                    val realPos = frame.blockPos
                    val blockBehind = realPos.offset(frame.horizontalFacing.opposite, 1)
                    ...
                    val column = when {
                        mc.world!!.getBlockState(blockBehind.offset(frame.horizontalFacing.rotateYCounterclockwise())).block != Blocks.IRON_BLOCK -> 2
                        mc.world!!.getBlockState(blockBehind.offset(frame.horizontalFacing.rotateYClockwise())).block != Blocks.IRON_BLOCK -> 0
                        else -> 1
                    }
                    val mapData = FilledMapItem.getMapState(frame.heldItemStack, mc.world) ?: continue
                    val colorInt: Int = (mapData.colors[8256] and 255.toByte()).toInt()
                    val owner = if (colorInt == 114) Board.State.X else Board.State.O
                    if (board == null) {
                        topLeft = realPos.up(row).offset(frame.horizontalFacing.rotateYClockwise(), column)
                        roomFacing = frame.horizontalFacing.opposite
                        board = Board()
                    }
                    with(board!!) {
                    ...
                    if (turn == Board.State.O) {
                        for (frame in frames) {
                            if (!mappedPositions.containsValue(frame)) {
                                val mapData =
                                    FilledMapItem.getMapState(frame.heldItemStack, mc.world) ?: continue
                                val colorInt: Int = (mapData.colors[8256] and 255.toByte()).toInt()
                                val owner = if (colorInt == 114) Board.State.X else Board.State.O
                                val realPos = frame.blockPos
                                val blockBehind = realPos.offset(frame.horizontalFacing.opposite, 1)
                                with(board!!) {
                                    ...
                                    val column =
                                        if (mc.world!!.getBlockState(blockBehind.offset(frame.horizontalFacing.rotateYCounterclockwise())).block != Blocks.IRON_BLOCK) {
                                            2
                                        } else {
                                            if (mc.world!!.getBlockState(blockBehind.offset(frame.horizontalFacing.rotateYClockwise())).block != Blocks.IRON_BLOCK) {
                                                0
                                            } else 1
                                        }
                                    place(column, row, owner)
