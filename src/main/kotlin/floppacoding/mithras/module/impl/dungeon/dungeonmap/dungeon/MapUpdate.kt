...
import floppacoding.mithras.module.impl.dungeon.dungeonmap.utils.MapUtils.decor
...
object MapUpdate {
     ...
     */
    fun updatePlayers(tabEntries: List<Pair<PlayerListEntry, String>>) {
        var iconNum = 0
        // Update teammate list and dead status
        for (i in listOf(5, 9, 13, 17, 1)) {
            val tabText = Formatting.strip(tabEntries[i].second)!!.trim()
            ...
            if ((teammate == null) || teammate.fakeEntity) {
                val potPlayer = mc.world?.players?.find { it.name.string == name }
                val fake = potPlayer == null
                (potPlayer ?: OtherClientPlayerEntity(mc.world, tabEntries[i].first.profile))
                    .let {
                        if (teammate == null){
                            Dungeon.dungeonTeammates.add(DungeonPlayer(it, name, fake))
                            // The skin texture could also be passed here!?
                        }else{
                            teammate!!.player = it
                            teammate!!.name = name
        ...
        val decor = MapUtils.getMapData()?.decor
...
//                        MinecraftForge.EVENT_BUS.post(DungeonRoomStateChangeEvent(tile, newState))
    ...
    private fun shouldScanMapItem() = !Dungeon.fullyScanned && !Dungeon.inBoss && RunInformation.currentFloor != null
