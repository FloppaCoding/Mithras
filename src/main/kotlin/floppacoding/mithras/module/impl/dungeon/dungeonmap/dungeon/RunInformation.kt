...
object RunInformation {
    ...
    val deathsPattern = Regex("Deaths: (?<deaths>\\d+)")
    val puzzleCountPattern = Regex("Puzzles: \\((?<count>\\d)\\)")

    /**
     * Matches all three states a puzzle can be in (unfinished, completed, failed).
     */
    val puzzlePattern = Regex(" (?<puzzle>.+): \\[(?<state>[✔✖✦?])]")
    val failedPuzzlePattern = Regex(" (?<puzzle>.+): \\[✖]")
    val solvedPuzzlePattern = Regex(" (?<puzzle>.+): \\[✔]")
    val secretsFoundPattern = Regex(" Secrets Found: (?<secrets>\\d+)")
    val secretsFoundPercentagePattern = Regex(" Secrets Found: (?<percentage>[\\d.]+)%")
    val cryptsPattern = Regex(" Crypts: (?<crypts>\\d+)")
    // For use with the scoreboard
    val dungeonClearedPattern = Regex("Cleared: (?<percentage>\\d+)% \\((?<score>\\d+)\\)")
    // For use with the scoreboard. THe time is also in the tab list but differently formatted.
    val timeElapsedPattern = Regex("Elapsed: (?:(?<hrs>\\d+)h )?(?:(?<min>\\d+)m )?(?:(?<sec>\\d+)s)?")
     ...
     */
    fun updateRunInformation(tabEntries: List<Pair<PlayerListEntry, String>>) {
