 ...
 *
 * // TODO update this information to 1.20
 * // TODO check whether the methods work
 *
...
object InventoryUtils {
    ...
    fun findItem(regex: Regex, inInv: Boolean = false, mode: Int = 0): Int? {
        return findItem(inInv) {
            it?.run {
                when (mode) {
                    0 -> it.name.string.contains(regex) || itemID.matches(regex)
                    1 -> it.name.string.contains(regex)
     ...
     *
     */
    fun ClientPlayerEntity?.isHoldingInMainHand(vararg items: SkyblockItem): Boolean {
        val regex = Regex(items.joinToString("|") { it.itemID })
        return this.isHoldingInMainHand(regex, 2)
    }
     ...
     * @param mode Specify what to check. 0: display name and item id. 1: only display name. 2: only itemID.
     */
    fun ClientPlayerEntity?.isHoldingInMainHand(vararg names: String, ignoreCase: Boolean = false, mode: Int = 0): Boolean {
        val regex = Regex("${if (ignoreCase) "(?i)" else ""}${names.joinToString("|")}")
        return this.isHoldingInMainHand(regex, mode)
    }
     ...
     * @param mode Specify what to check. 0: display name and item id. 1: only display name. 2: only itemID.
     */
    fun ClientPlayerEntity?.isHoldingInMainHand(regex: Regex, mode: Int = 0): Boolean {
        return this.isHoldingInMainHand { it?.run {
            when (mode) {
                0 -> it.name.string.contains(regex) || itemID.matches(regex)
                1 -> it.name.string.contains(regex)
                2 -> itemID.matches(regex)
     ...
     */
    fun ClientPlayerEntity?.isHoldingInMainHand(predicate: (ItemStack?) -> Boolean): Boolean {
        if (this == null) return false
        return predicate(this.mainHandStack)
    }
    //</editor-folding>
