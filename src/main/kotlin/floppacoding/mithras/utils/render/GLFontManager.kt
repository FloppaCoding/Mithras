...
object GLFontManager {
    ...
    val KURINTO: GLFont =
        GLFont("roboto", "/assets/${Mithras.RESOURCE_DOMAIN}/gui/fonts/KurintoSans-Rg.ttf")
    ...
    class GLFont(val name: String, val path: String) : Font {
        ...
        val glyphMetrics2: MutableMap<Char, GlyphMetrics> = mutableMapOf()
        val glyphMetrics3: MutableMap<Char, GlyphMetrics> = mutableMapOf()

        lateinit var fontMetrics2: FontMetrics
        lateinit var fontMetrics3: FontMetrics
        ...
        private fun createFont(): Int {
            val bitmap = MemoryUtil.memAlloc(512*512)
            val bakedChars = STBTTBakedChar.malloc(256-32)
            val result = stbtt_BakeFontBitmap(fontBuffer, 32f, bitmap, 512, 512, 32, bakedChars)
            // TODO can free fontBuffer from  here on!
        ...
        private fun createFont2(): Int {
            ...
            val x1: IntBuffer = MemoryUtil.memAllocInt(1)
            val y1: IntBuffer = MemoryUtil.memAllocInt(1)
            ...
            val scale = stbtt_ScaleForPixelHeight(fontInfo, 32f)
            stbtt_GetFontVMetrics(fontInfo, ascent, descent, lineGap)

            fontMetrics2 = FontMetrics(32f, ascent[0]*scale, descent[0]*scale, lineGap[0]*scale)

            val baseline = 32
            val lineOffset = ((ascent[0] - descent[0] + lineGap[0])*scale).toInt()

            for (char in SYMBOLS) {
                val code = char.code
                stbtt_GetCodepointHMetrics(fontInfo, code, advance, leftSideBearing)

                stbtt_GetCodepointBitmapBox(fontInfo, code, scale, scale, x0, y0, x1, y1)
                if (xPos[0] + x1[0] - x0[0] >= BMP_WIDTH ) {
                    yPos.put(0, yPos[0] + lineOffset)
                    xPos.put(0, 0)
                }
                val newPos = (baseline + y0[0] + yPos[0])*BMP_WIDTH + xPos[0] + x0[0]

                bitmap.position(newPos)
                stbtt_MakeCodepointBitmap(fontInfo, bitmap, x1[0] - x0[0], y1[0] - y0[0], BMP_WIDTH, scale, scale, code)
                // TODO make sure there are no overlaps, which override eachother



                // Save metrics
                glyphMetrics2[char] = GlyphMetrics(
                    (xPos[0] + x0[0]).toFloat()/BMP_WIDTH,
                    (baseline -ascent[0]*scale + yPos[0]) /BMP_HEIGHT,
                    (xPos[0] + x1[0]).toFloat()/BMP_WIDTH,
                    (baseline -descent[0]*scale + yPos[0]) /BMP_HEIGHT,
                    (x1[0] - x0[0]).toFloat(),
                    advance[0]*scale,
                    leftSideBearing[0]*scale,
                )

                if (xPos[0] + (advance[0]*scale).toInt() + 1 >= BMP_WIDTH ) {
                    yPos.put(0, yPos[0] + lineOffset)
                    xPos.put(0, 0)
                }else {
                    xPos.put(0, xPos[0] + 1 + (advance[0]*scale).toInt())
                }

                if (yPos[0] > 2500)
                    break

//                if (!mergeBMPs(bitmap, glyphBMP, xPos, yPos, 1024, ix1[0]-ix0[0], iy1[0] - iy0[0], 33)) break
//                xPos.put(0, xPos[0] +1)
        ...
        private fun createFont3(): Int {
            ...
            // Allocate memory
            val fontInfo = STBTTFontinfo.create()
            val glyphBMP = MemoryUtil.memCalloc(32*32)
            val bitmap = MemoryUtil.memCalloc(BMP_WIDTH*BMP_HEIGHT)
            val x0: IntBuffer = MemoryUtil.memAllocInt(1)
            val y0: IntBuffer = MemoryUtil.memAllocInt(1)
            ...
            stbtt_InitFont(fontInfo, fontBuffer)
            val scale = stbtt_ScaleForPixelHeight(fontInfo, 26f)
            val padding = 3
            val onEdgeInt = 128
            val on_edge: Byte = onEdgeInt.toByte()
            // with this negative values smaller than on_Edge are inside
            val dist_scale = (255-onEdgeInt).toFloat() / padding
            stbtt_GetFontVMetrics(fontInfo, ascent, descent, lineGap)

            fontMetrics3 = FontMetrics(26f, ascent[0]*scale, descent[0]*scale, lineGap[0]*scale)

            val baseline = 32
            val lineOffset = ((ascent[0] - descent[0] + lineGap[0])*scale).toInt() + padding * 2

            for (char in SYMBOLS) {
                val code = char.code
                stbtt_GetCodepointHMetrics(fontInfo, code, advance, leftSideBearing)

                val sdf = stbtt_GetCodepointSDF(fontInfo, scale, code, padding, on_edge, dist_scale,  width, height, x0, y0)
                if (sdf == null) {
                    // Handle space
                    if (char == ' ') {
                        glyphMetrics3[char] = GlyphMetrics(
                            0f,
                            0f,
                            0f,
                            0f,
                ...
                val newPos = (baseline + yPos[0] + y0[0])*BMP_WIDTH + xPos[0]
                ...
                // Save metrics
                glyphMetrics3[char] = GlyphMetrics(
                    (xPos[0] + padding).toFloat()/BMP_WIDTH,
                    (baseline - ascent[0]*scale + yPos[0]) /BMP_HEIGHT,
                    (xPos[0] + width[0]-padding).toFloat()/BMP_WIDTH,
                    (baseline -descent[0]*scale + yPos[0]) /BMP_HEIGHT,
                    (width[0]- 2*padding).toFloat(),
                    advance[0]*scale,
                    leftSideBearing[0]*scale,
                )
...
//                xPos.put(0, xPos[0] +1)
         ...
         */
        fun mergeBMPs(bitmap: ByteBuffer, new: ByteBuffer, stride: Int, width: Int, height: Int): Boolean {
        ...
        class FontMetrics(
            /**
             * The size of the underlying glyphs.
             */
            val size: Float,
            /**
            ...
            val ascent: Float,
            /**
             * How far letters like g might extend below the baseline. This is negative.
             */
            val descemt: Float,
            val lineGap: Float,
        )
